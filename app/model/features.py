...
def get_trades_indexes(books, trades, offset, live=False):
    ...
    def indexes(ts):
        ts = int(ts)*1000
        i_0 = trades.timestamp.searchsorted([ts-(offset*1000)], side='left')[0]
        if live:
            i_n = -1
        else:
...
def make_features(symbol, sample, mid_offsets,
    ...
    if not live:
        print('trade features run time:', (time.time()-stage)/60, 'minutes')
        stage = time.time()
        print('make_features run time:', (time.time()-start)/60, 'minutes')
...
if __name__ == '__main__' and len(sys.argv) == 3:
    ...
    data.to_csv('data/'+time.strftime("%Y%m%d-%H%M%S")+".csv", sep=';')
    # with open('data/'+time.strftime("%Y%m%d-%H%M%S")+".pkl", 'w+') as file:
